...
int sender(int sockfd, char* data, int data_size, int flags,
           ...
           
    {
        ...
        // to determine the last packet size to be sent in case if bytes to send doesnt divide with 16 exactly.
        ...
        while ((last_ack_received < num_frames)) 
        {
            ...
            {
                //frame size determines the size of packet. In general last packet to be sent varies in size.
            ...
            // wait for 700msec. THis is double the max RTT that my machine has provided.
            while(1)
            {
            ...
            // actually data has been sent and receiver sends the ack number. Then last_ack_received is accordingly set for next iteration.
...
int receiver(int sockfd, char* received_data, int max_data_size, int flags,
             struct sockaddr* src_addr, socklen_t* addrlen) 
    {
        ...
        // receives the size of the message arriving and stores it in specific location.
        ...
        //     set_count=set_count-1;
        // }
        // to determine the last packet size to be received in case if bytes to send doesnt divide with 16 exactly.
        int last_packet_size;
        ...
        while (expected_frame < num_frames) 
        {
            ...
            for(int i=0;i<4;i++)
            {
                ...
                // if any packet did not get valid bytes then we consider packet is not received.
                if (bytes_received < 0) 
                {
            ...
            for (int i = 0; i < 4; i++) 
            {
                // if 1st packet itself not received then expected frame went back to normal.
