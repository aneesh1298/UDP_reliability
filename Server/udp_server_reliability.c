...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    bzero(buf, BUFSIZE);
    n = receiver(sockfd, buf, BUFSIZE, 0,
		 (struct sockaddr *) &clientaddr, &clientlen);
    if (n < 0)
      error("ERROR in receiver");
    n = receiver(sockfd, command, BUFSIZE, 0,
		 (struct sockaddr *) &clientaddr, &clientlen);
    if (n < 0)
      error("ERROR in receiver");
    n = receiver(sockfd, file_name, BUFSIZE, 0,
		 (struct sockaddr *) &clientaddr, &clientlen);
    if (n < 0)
      error("ERROR in receiver");
    ...
    if(!(strcmp(command,"get")))
    {
        ...
        if(file_read== NULL)
        {
          ...
          n = sender(sockfd, buf, 1024, 0,(struct sockaddr *) &clientaddr, clientlen);
          if (n < 0)
        ...
        // have to send file data 1024 bytes at atime.
            ...
            for(int cumulatives_bytes_transferred=0; file_size>=cumulatives_bytes_transferred;cumulatives_bytes_transferred+=byte_transfer_size)
            {
              ...
              n = sender(sockfd, buf, bytes_sent, 0, (struct sockaddr *)&clientaddr, clientlen);
              if (n < 0)
                error("ERROR in sender");
    ...
    else if(!(strcmp(command,"put")))
    {
          ...
          n = receiver(sockfd, buf, 1024, 0,  (struct sockaddr *)&clientaddr, &clientlen);
          if (n < 0) 
            error("ERROR in receiver");
    ...
    else if( ! (strcmp(command,"delete")))
    {
        ...
        if(file_read== NULL)
        {
          ...
          strcpy(buf, "donotexist");
          printf("The file %s requested  DOES NOT EXIST \n", file_name);
          n = sender(sockfd, buf, strlen(buf), 0,(struct sockaddr *) &clientaddr, clientlen);
          if (n < 0)
            error("ERROR in sender");
          continue;
        }
    ...
    else
    {
        ...
        n = sender(sockfd, command, 1024, 0, 
	       (struct sockaddr *) &clientaddr, clientlen);
        if (n < 0) 
          error("ERROR in sender");
    }
